...
class BillOfMaterial(TenantAwareModel):
    """Bill of Materials (BOM) - Lista de Materiales — Tenant-Isolated"""
...
class BOMLine(models.Model):
    """BOM Line Item - inherits tenant via BOM FK (no direct company field needed)"""
...
class WorkOrder(TenantAwareModel):
    """Work Order - Orden de Trabajo — Tenant-Isolated"""
    STATUS_CHOICES = [
        ('pending', 'Pendiente'),
    ...
    work_order_number = models.CharField(max_length=50, verbose_name="Número de Orden")
    product = models.ForeignKey('logistica.Product', on_delete=models.PROTECT, verbose_name="Producto")
    ...
    class Meta:
        ...
        ordering = ['-start_date']
        unique_together = [['company', 'work_order_number']]
